...
#   }
  ...
  filename=paste("Sunchart-",type,"-",lang,"-",location.name,".",out,sep="")
  if (out=="png") {
    ...
    Cairo::CairoSVG(file=filename,width=11,height=11)
  }
  ...
  if (type=="cart") {
    ...
    legend(x=45,y=min(90-phi*180/pi+31,90),legend=location(lat=phi,long=lambda,name=location.name),box.col="white",bg="white",cex=1.7)
    ...
    legend("topleft", legend=location(lat=phi, long=lambda, name=location.name), box.col="white", bg="white", cex=1)
  }
